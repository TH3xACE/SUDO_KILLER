...
sudonopassuser=`echo '' | sudo -S -l -k 2>/dev/null | grep "NOPASSWD:" | grep -v "root" | sed 's/NOPASSWD//g' | sed 's/(//g' | sed 's/)//g' | sed 's/://g' `
...
if [ "$sudopwfeedback" ]; then
  ...
  echo -e "[-] Vulnerable to CVE-2019-18634 if the sudo version is 1.7.1 to 1.8.25p1 inclusive, check the version of sudo"
...
if [ "$2" = "root" ];
then    
var2=`echo '' | sudo -S -l -k 2>/dev/null | grep "(root) NOPASSWD:" | grep -w "bin/$1"`
if [ "$var2" ]; then
...
if [ "$2" = "other" ];
then
var2=`echo '' | sudo -S -l -k 2>/dev/null | grep ") NOPASSWD:" | grep -v "root" | grep -w "bin/$1"`


if [ "$var2" ]; then
echo -e "[+] Sudo $1, was found "
usr=`echo '' | sudo -S -l -k 2>/dev/null | grep ") NOPASSWD:" | grep -v "root" | grep -w "bin/$1" | cut -d ")" -f 1 | sed 's/(//g' `
path1=`echo '' | sudo -S -l -k 2>/dev/null | grep ") NOPASSWD:" | grep -v "root" | grep -w "bin/$1" | cut -d ":" -f 2 | sed 's/(//g' `
