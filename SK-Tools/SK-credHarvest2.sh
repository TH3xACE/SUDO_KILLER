...
checkinitial() {
  ...
  echo -e "${BOLD}${YELLOW}[1/20] ====== Initial check - Quick overview ====== ${RESET} \n"
...
if [ "$sudocve" ]; then
    echo -e "${BOLD}${YELLOW}[2/20] ====== Checking for disclosed vulnerabilities (CVEs) - version based ====== ${RESET} \n"
    check_cve_version 
    echo -e "${BOLD}${BLUE}[@] Check [2/20] Completed!${RESET} \n"

# ---------------------------------------------

    echo -e "${BOLD}${YELLOW}[3/20] ====== Checking for disclosed vulnerabilities (CVE) ====== ${RESET} \n"
...
if [ "$sudoescapevschk" ]; then
  ...
  if [ "$sudounescapeof" ]; then
   ...
   if [ "$sudo_escape" ]; then
    ...
    echo -e "${BOLD}${BLUE}[@] Check [3/20] Completed!${RESET} \n"
else
 echo -e "${BOLD}${YELLOW}[2/20] ====== Checking for disclosed vulnerabilities (CVEs) - version based ====== ${RESET} \n"
...
# regex for /etc/sudoers | grep -E '^\S+\s+\(ALL\)\s+ALL'
 exprv11=$(echo "$cmd" 2>/dev/null | grep -E '\(ALL\)\s+ALL')
 if [ "$exprv11" ]; then
  echo -e "${BOLD}${RED}[+] Excessive Privilege detected [Variant 1]${RESET}"
...
# regex for /etc/sudoers | grep -E '^\S+\s+\(ALL\)\s+/usr/bin/\w+'
 exprv451=$(echo "$cmd" 2>/dev/null | grep -E '\(ALL\)\s+/*bin*')
 if [ "$exprv451" ]; then
  echo -e "${BOLD}${RED}[+] Excessive Privilege detected [Variant 4/5]${RESET}"
...
# # regex for /etc/sudoers | grep -E '^%\w+\s+\(ALL\)\s+ALL'
 exprv21=$(echo "$cmd" 2>/dev/null | grep -E '\(ALL \: ALL\)\s+ALL')
 if [ "$exprv21" ]; then
  echo -e "${BOLD}${RED}[+] Excessive Privilege detected [Variant 2]${RESET}"
...
# regex for /etc/sudoers | grep -E '^%\w+\s+\(ALL\)\s+/*bin/\*'
 exprv61=$(echo "$cmd" 2>/dev/null | grep -E '\(ALL \: ALL\)\s+/*bin*')
 if [ "$exprv61" ]; then
  echo -e "${BOLD}${RED}[+] Excessive Privilege detected [Variant 6]${RESET}"
...
if [ "$sudowildcardchmod" ]; then
 ...
 checkmisconfig()
 {
   echo -e "${BOLD}${YELLOW}[4/20] ====== Checking for excessive privilege ====== ${RESET} \n"
   fn_excess_priv 

   echo -e "${BOLD}${BLUE}\n[@] Check [4/20] Completed!${RESET} \n"
   
   echo -e "${BOLD}${YELLOW}[5/20] ====== Checking for excessive privilege (a password required) ====== ${RESET} \n"
   fn_excess_priv_pwd 

   echo -e "${BOLD}${BLUE}\n[@] Check [5/20] Completed!${RESET} \n"
 
   echo -e "${BOLD}${YELLOW}[6/20] ====== Checking for Common Misconfiguration (User impersonation) ====== ${RESET} \n"
   fn_impersonate
   echo -e "${BOLD}${BLUE}\n[@] Check [6/20] Completed!${RESET} \n"
 
   echo -e "${BOLD}${YELLOW}[7/20] ====== Checking for Common Misconfiguration (Change owner) ====== ${RESET} \n"
   fn_chown
   echo -e "${BOLD}${BLUE}\n[@] Check [7/20] Completed!${RESET} \n"
  
   echo -e "${BOLD}${YELLOW}[8/20] ====== Checking for Common Misconfiguration (Wildcard) ====== ${RESET} \n"
   fn_wildcard
   echo -e "${BOLD}${BLUE}\n[@] Check [8/20] Completed!${RESET} \n"
   
   echo -e "${BOLD}${YELLOW}[9/20] ====== Checking for Common Misconfiguration (Sudo Injection) ====== ${RESET} \n"
   fn_sinject
   echo -e "${BOLD}${BLUE}\n[@] Check [9/20] Completed!${RESET} \n"
  
   echo -e "${BOLD}${YELLOW}[10/20] ====== Checking for Common Misconfiguration (Sudo Cache) ====== ${RESET} \n"
   fn_scache
   echo -e "${BOLD}${BLUE}\n[@] Check [10/20] Completed!${RESET} \n"

   echo -e "${BOLD}${YELLOW}[11/20] ====== Checking for Common Misconfiguration (File Owner Hijacking) ====== ${RESET} \n"
   fn_fileownhijack
   echo -e "${BOLD}${BLUE}\n[@] Check [11/20] Completed!${RESET} \n"  

  echo -e "${BOLD}${YELLOW}[12/20] ====== Checking for Common Misconfiguration (File Permission Hijacking) ====== ${RESET} \n"
  fn_filepermhijack
  echo -e "${BOLD}${BLUE}\n[@] Check [12/20] Completed!${RESET} \n"  
...
cat $vpath/script_list | while read linex
do
...
checkmissing()
{
  ...
  echo -e "${BOLD}${YELLOW}[13/20] ====== Checking for Missing scripts from sudo's rules ====== ${RESET} \n"
  fn_miss_scripts
  echo -e "${BOLD}${BLUE}\n[@] Check [13/20] Completed!${RESET} \n"  

  echo -e "${BOLD}${YELLOW}[14/20] ====== Checking for excessive directory's permission ====== ${RESET} \n"
  fn_excessive_dir_perm
  echo -e "${BOLD}${BLUE}\n[@] Check [14/20] Completed!${RESET} \n"

  echo -e "${BOLD}${YELLOW}[15/20] ====== Checking for Writable scripts from sudo's rules ====== ${RESET} \n"
  fn_writeable_script_perm
  echo -e "${BOLD}${BLUE}\n[@] Check [15/20] Completed!${RESET} \n"  
...
checkcredharvest(){
  ...
  echo -e "${BOLD}${YELLOW}[16/20] ====== Checking for backdooring sudo (Credentials Capture) ====== ${RESET} \n"
  fn_cred_harvest
  echo -e "${BOLD}${BLUE}\n[@] Check [16/20] Completed!${RESET} \n" 
...
echo -e "${BOLD}${YELLOW}[18/20] ====== Checking for Dangerous binaries (gtfobins) ====== ${RESET} \n"
...
cat $PWD/res/Dangerous_env_var.txt | while read linen1
do
...
echo -e "\n"
 ...
 checkdangenvar()
 {
  echo -e "${BOLD}${YELLOW}[17/20] ====== Checking for Dangerous Environment Variables ====== ${RESET} \n"
  fn_dang_envar
  echo -e "${BOLD}${BLUE}\n[@] Check [17/20] Completed!${RESET} \n" 
...
checkrecurimper(){
  echo -e "${BOLD}${YELLOW}[19/20] ====== Checking for recursive user impersonation (depth:3) ====== ${RESET} \n"
  fn_recur_impersonate
  echo -e "${BOLD}${BLUE}\n[@] Check [19/20] Completed!${RESET} \n" 
